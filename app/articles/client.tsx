        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="flex flex-col md:flex-row gap-sm mb-lg">
            <input
              ...
              className="w-full md:max-w-sm px-sm py-xs rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-50 placeholder-zinc-600 focus:border-zinc-600 focus:outline-none transition-colors text-sm"
            />
          </div>

          <div className="flex flex-wrap gap-2xs mb-xl">
            <button
              onClick={() => setSelectedCategory("all")}
              className={`px-xs py-1.5 rounded-full text-eyebrow font-mono transition-all ${
          ...
          {loading ? (
            ...
            <div className="py-xl text-center">
              <p className="text-zinc-500 mb-sm">No articles found.</p>
              <button
                onClick={() => { setSearchTerm(""); setSelectedCategory("all") }}
                className="text-zinc-300 text-sm border border-zinc-700 rounded-full px-sm py-2xs hover:text-zinc-50 hover:border-zinc-500 transition-all"
              >
                Clear filters
              </button>
            </div>
            ...
            <div className="space-y-0">
              {displayArticles.map((article, i) => (
                <Link
                  ...
                  className={`group flex flex-col md:flex-row md:items-center gap-xs py-sm ${
                    i !== displayArticles.length - 1 ? 'border-b border-zinc-800' : ''
                  } hover:pl-2 transition-all duration-200`}
                >
                  <span className="text-eyebrow font-mono text-zinc-600 tabular-nums shrink-0 w-20">
                    {article.pubDate ? new Date(article.pubDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : ''}
                  </span>
                  <h3 className="text-xl md:text-2xl font-display text-zinc-300 group-hover:text-zinc-50 transition-colors flex-1 leading-snug">
                    {article.title}
                  </h3>
                  {article.categories && article.categories.length > 0 && (
                    <div className="flex gap-2xs shrink-0">
                      {article.categories.slice(0, 2).map((cat: string, ci: number) => (
                        <span key={ci} className="text-micro font-mono text-zinc-600 border border-zinc-800 rounded px-2xs py-0.5">
                          {cat}
                        </span>
                      ))}
                    </div>
